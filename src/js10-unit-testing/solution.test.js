...
test("Should return new array of names based on length", () => {
  expect(filterByLength(["Paul", "Calum", "Rebecca"])).toMatchObject([
    "Paul",
    "Calum",
  ]);
  expect(filterByLength(["Johnathan", "Sylvester", "Charlie"])).toBe(
    "Sorry, no valid names supplied"
  );
  expect(filterByLength(["Tom", "Dick", "Harry"])).toMatchObject([
    "Tom",
    "Dick",
    "Harry",
  ]);
